...
def queryCaidan(request):
    ...
    for it in feed:
        ...
        if item:
            item = item[0]
            feed_list.append({'name':item.name,'src':str(item.src),'numb':0, 'price':item.price})
    ...
    for it in typeName:
        ...
        for item in menuContent:
            list.append({'name':item.name,'src':str(item.src),'numb':0, 'price':item.price})
